# statements_seed\ — starting points for the launch template suite
...
**This folder is NOT loaded by the app.** The app only reads `..\statements\`
(shipped) and `..\statements_user\` (built on the box). Nothing in here affects detection or
...
4. Confirm the balance reconciles, then **Save**. It lands in `..\statements_user\`.
5. To ship it to the whole team, move the saved file into `..\statements\` and add
   a golden-file test (see `..\..\tests\HOWTO-add-template-test.md`).
