# extract_fields.R -- key-value ("mode: fields") extraction for documents whose useful data is
# labelled values, not a transaction table. A template declares FIELDS instead of columns, each a
# label-matcher spec (see R/labels.R). A form has NO reconciliation behind it, so conflict /
# other_values / n are the whole of its evidence - the disputing figure is the one thing a person
# holding the document can check in five seconds.
...
extract_fields <- function(input, template, dict = default_label_dict()) {
  ...
  rows <- lapply(names(fields), function(fn) {
    ...
    if (!is.null(region)) {
      # POSITIONAL extraction: the value is read from a box on the page, NOT by its label, for forms
      # where value and label are in very different places.
      ...
      # LABEL-based extraction. Inherit synonyms from the base dictionary: an explicit `dict:` key,
      # else the field's own name if it matches an entry.
      key <- spec$dict %||% fn
      if (!is.null(dict[[key]])) {
    ...
    # `occurrence: all` already puts every reading in the value itself, so repeating them would read
    # as a disagreement that is not there.
    ...
    data.frame(
      ...
      label    = label,
      value    = res$value,
      # The readings this value BEAT. Empty on a clean field, so it costs a clean form nothing.
      ...
      # How many times this label was read carrying a value - the count behind `conflict`.
...
# The page NUMBERS match_label will search, the exact mirror of .pages_in_scope(), which returns
# their TEXT. Two functions, one rule, and a test pins them to the same answer.
...
# The page a matched value was read from, or NA. match_label() joins every page in scope into ONE
# string before it looks for anything, and that join also decides which line wins, so it cannot be
# taken apart without changing which FIGURE comes out; it is rebuilt here line for line with the page
# beside each. The "x" is not a typo: strsplit() drops trailing empty fields, so a page ending in a
# newline would be counted one line short and every page after it named wrongly.
.field_page <- function(pages, spec, res) {
  ...
  # The same line printed on two pages: which one was taken follows the same rule match_label used to
  # choose the value - last for `occurrence: last` and `on_conflict: last`, the first otherwise.
...
# Read the text inside a page-box in reading order and coerce it to the requested value type. The
# label-free counterpart to match_label.
