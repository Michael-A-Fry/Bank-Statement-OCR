# inspect.R -- geometry for the "Statement X-ray": show, on the page image, exactly what the engine
# selects and where every value is pulled from. Everything is in PDF points, top-left origin - the
# same space word boxes, x-bands and the rendered raster share.

# Name of the first column band containing centre-x cx, or NA. Mirrors .pdf_cell's test exactly.
...
# Which words fall in the table region - the same four filters parse_pdf_table applies.
...
# Per-page overlay geometry. `column` is which template column a word is selected into; `rows$kept`
# marks a row the engine would keep as a transaction.
inspect_pdf_layout <- function(input, template, force_rows = NULL) {
  ...
  # keep_dateless_rows is the reader's fourth keep branch. Read it HERE too, or the overlay paints
  # rows the reader keeps as SKIPPED and row_coverage blames a date format that is not the problem.
  ...
  # Bands live in THE BAND FRAME. The X-ray paints on each page's rendered raster, in that page's OWN
  # point space, so it DIVIDES each band by the frame scale and the reader does the exact mirror with
  # the SAME function. Hand-rolling it here snapped on the reciprocal ratio, so for a page 2% off the
  # frame the X-ray drew a band 10pt from where the reader matched it.
  ...
  # metadata_regions belonging on page p, scaled into page space, drawn as pinned header-value boxes.
  ...
  pages <- lapply(seq_along(wbp), function(p) {
    ...
    # ocr_conf: per-word OCR confidence on a machine-read page, NA on a text-layer page - always
    # present so the app can shade doubtful words.
    ...
    if (nrow(rw)) {
      ...
      info <- lapply(unique(grp), function(g) {
        ...
        # Apply the ENGINE's full keep predicate, not just the date test: a dated line still has to
        # carry a money amount and not be a summary line. pdf_keep_row IS that predicate.
        ...
        # A row the user forced in is painted kept here too.
        forced <- .forced_band_hit(p, min(rg$y), max(rg$y + rg$height), fr_p)
        kept <- natural_keep || forced
        # Why the engine did NOT keep this row, from the same helper the engine uses, so the X-ray
        # can never explain it differently.
  ...
  # Year-less fallback, mirrored from parse_pdf_table so the X-ray never disagrees with the reader:
  # when the template's own format reads ZERO dates but the period supplies the year, rows the overlay
  # marked "date didn't parse" must flip to kept.
  ...
  if (!prim_any && length(all_dates)) {
    ...
    yr <- unique(yr)
    # With no year found anywhere, mirror the reader's sentinel keep: a clear name-month cell still
    # keeps its row, flagged upstream.
...
# Keep just the leading date piece, so a two-date band still validates the row's date.
...
# Mirror the reader's SPLIT-ROW recovery. Without this the X-ray showed BOTH halves as skipped while
# the reader had already recovered them - an actionable-looking count for rows never lost.
...
# Upgrade a dropped "no date and no amount" row to "continuation" when it sits directly under a KEPT
# transaction. It also GROWS the parent's box down over the folded line, so the kept rectangle
# visibly includes the wrapped description.
...
# For each labelled value, the bounding box of the contiguous run of page words matching its tokens,
# so the view can draw a box around where the value came from. found = FALSE when not located.
