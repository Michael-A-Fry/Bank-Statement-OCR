...
Rscript -e "pkgs<-c('shiny','DT','yaml','jsonlite','openxlsx','readxl','pdftools','magick','testthat'); miss<-pkgs[!vapply(pkgs,requireNamespace,logical(1),quietly=TRUE)]; if(length(miss)){install.packages(miss,repos='https://cloud.r-project.org')} else cat('all packages present\n')"
