...
| [ird-document-plan.md](ird-document-plan.md) | Generalising `mode: document` for IRD reports: the running score against the source analysis. What is built (the filing-period matrix), what is **not** (row reconstruction, components, the other two orientations), the parallel branch it must be merged with, and why no IRD template ships yet. |
